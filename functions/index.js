...
// --- Rate Limiting Configaduraation ---
...
//     }
// });
async function handleDeleteUserAccount(payload, userId) {
...
async function executeUserDeletion(userId) {
    ...
    try {
        // Phase 1: Delete all user's posts and their subcollections
        console.log(`🗑️ [AUDIT] Phase 1: Deleting user's posts for ${userId}`);
        await deleteUserPosts(userId, deletionStats);

        // Phase 2: Clean up following/follower relationships
...
async function cleanupFollowRelationships(userId, stats) {
    try {
        // Get users who follow this user (their followers)
        const userRef = db.collection(COLLECTIONS.USERS).doc(userId);
        ...
        // Remove this user from each follower's "following" list and decrement their following count
        for (const followerDoc of followersSnapshot.docs) {
            try {
                const followerId = followerDoc.id;
                const followerUserRef = db.collection(COLLECTIONS.USERS).doc(followerId);

                // Remove from their following collection and decrement count
                await db.runTransaction(async (transaction) => {
        ...
        // Get users this user follows (their following)
        ...
        // Remove this user from each followed user's "followers" list and decrement their follower count
        for (const followingDoc of followingSnapshot.docs) {
            try {
                ...
                // Remove from their followers collection and decrement count
                await db.runTransaction(async (transaction) => {
...
async function deleteUserLikes(userId, stats) {
    try {
        ...
        // Delete each like and decrement post's like count
        for (const likeDoc of likesSnapshot.docs) {
            try {
                // Extract post ID from the document path
                const pathParts = likeDoc.ref.path.split('/');
                const postId = pathParts[1]; // Path is: posts/{postId}/likes/{userId}
...
async function deleteUserComments(userId, stats) {
    try {
        // Use collection group query to find all comments by this user across all posts
        ...
        // Delete each comment and decrement post's comment count
        for (const commentDoc of commentsSnapshot.docs) {
            try {
                // Extract post ID from the document path
                const pathParts = commentDoc.ref.path.split('/');
                const postId = pathParts[1]; // Path is: posts/{postId}/comments/{commentId}
...
async function deleteUserBookmarks(userId, stats) {
    try {
        // Get all bookmarks this user created
        const userRef = db.collection(COLLECTIONS.USERS).doc(userId);
        ...
        // Delete each bookmark and decrement post's bookmark count
        for (const bookmarkDoc of bookmarksSnapshot.docs) {
            try {
                const postId = bookmarkDoc.id; // Document ID is the post ID
